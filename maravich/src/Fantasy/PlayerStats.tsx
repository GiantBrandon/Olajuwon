...
export const PlayerStats: React.FC<PlayerStatsProps> = (props) => {
    ...
    const displayStat = (label: String, playerValue: number, lowvalue: number, highValue: number) => {
        if(playerValue <= lowvalue)
            return <RedStat>{label}: {playerValue}</RedStat>
        else if (playerValue >= highValue)
            return <GreenStat>{label}: {playerValue}</GreenStat>
        else
            return <p>{label}: {playerValue}</p>
    }
	return (
		<>
