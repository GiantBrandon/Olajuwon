...
export const ShipSelectionGrid: React.FC<ShipSelectionGridProps> = ({ game }) => {
  ...
  return (
    ...
    <GridWrapper onMouseLeave={() => setHover(-10)} onKeyDown={(e) => handleKey(e)} tabIndex={0}>
      <GridView container columns={10}>
        {[...Array(100)].map((_, index) => <BattleCell key={index} size={'medium'} status={placedShips.includes(index) ? 'Ship' : 'Untouched'} hover={hovered ? hovered.includes(index) : false} setHover={() => setHover(index)} onClick={addShip} />)}
      </GridView>
    </GridWrapper>
